...
import { Zap, Activity, Droplet, AlertCircle } from 'lucide-react';

interface KeyDescriptorsGridProps {
    data: any;
}

export const KeyDescriptorsGrid: React.FC<KeyDescriptorsGridProps> = ({ data }) => {
    if (!data) {
        return (
            <Card className="bg-white/80 backdrop-blur-sm border-slate-200 shadow-md">
                <CardContent className="p-8 text-center text-slate-500">
                    Analyze a sequence to view key descriptors
                </CardContent>
            </Card>
        );
    }
