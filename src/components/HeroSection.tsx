...
import { Search, Zap, Activity, Droplet, AlertCircle, HelpCircle } from 'lucide-react';
...
// Residue color mapping based on properties
const RESIDUE_COLORS: Record<string, string> = {
    // Hydrophobic (gold/amber)
    'A': '#f59e0b', 'V': '#f59e0b', 'I': '#f59e0b', 'L': '#f59e0b', 'M': '#f59e0b', 'F': '#f59e0b', 'W': '#f59e0b', 'P': '#f59e0b',
    // Polar (blue/cyan)
    'S': '#3b82f6', 'T': '#3b82f6', 'N': '#3b82f6', 'Q': '#3b82f6', 'C': '#3b82f6', 'Y': '#3b82f6',
    // Acidic (red)
    'D': '#ef4444', 'E': '#ef4444',
    // Basic (purple/violet)
    'K': '#a855f7', 'R': '#a855f7', 'H': '#a855f7',
    // Glycine (gray)
    'G': '#94a3b8',
};
...
export const HeroSection: React.FC<HeroSectionProps> = ({ sequence, setSequence, onAnalyze, metrics }) => {
    ...
    // Colorize sequence in real-time
    const colorizedSequence = useMemo(() => {
        return sequence.split('').map((aa, i) => ({
            letter: aa,
            color: RESIDUE_COLORS[aa] || '#94a3b8',
            id: i
        }));
    }, [sequence]);
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Only allow valid amino acid characters
        const filtered = value.replace(/[^ACDEFGHIKLMNPQRSTVWY]/g, '');
        setSequence(filtered);
    };

    const isValidSequence = sequence.length > 0 && /^[ACDEFGHIKLMNPQRSTVWY]+$/.test(sequence);
    ...
    return (
        <section className="relative w-full min-h-[75vh] overflow-hidden">
            ...
            {/* Hero Content */}
            <div className="relative z-10 max-w-6xl mx-auto px-8 py-20 md:py-28">
                {/* Title Block */}
                <motion.div
                ...
                >
                    <h1 className="text-5xl md:text-6xl font-semibold text-blue-950 tracking-tight leading-tight mb-6"
                        style={{ fontFamily: 'Inter, SF Pro Display, system-ui', letterSpacing: '-0.5px' }}>
                        Peptide Intelligence Inspired by Biomimicry
                    </h1>
                    <p className="text-xl md:text-2xl text-blue-900/70 max-w-3xl mx-auto leading-relaxed font-normal"
                ...
                >
                    <div className="relative group">
                        {/* Glow effect */}
                        <div className={`absolute -inset-2 bg-gradient-to-r from-indigo-400 via-purple-400 to-cyan-400 rounded-3xl blur-xl opacity-30 transition-opacity duration-500 ${isFocused ? 'opacity-60' : 'group-hover:opacity-50'}`} />

                        {/* Input container */}
                        <div className="relative flex items-stretch bg-white/80 backdrop-blur-xl rounded-2xl border-2 border-white/40 shadow-2xl overflow-hidden">
                            {/* Search icon */}
                            ...
                            <div className="flex-1 py-5 pr-4 relative">
                                {colorizedSequence.length === 0 ? (
                                    <div className="text-lg text-slate-400 font-light"
                                        style={{ fontFamily: 'JetBrains Mono, monospace' }}>
                                        Enter peptide sequence (e.g., MADQLTEEQIAEFKEA)
                                    </div>
                                ) : (
                                    <div className="flex flex-wrap gap-1 text-lg font-mono leading-relaxed">
                                        {colorizedSequence.map((aa) => (
                                            <motion.span
                                                key={aa.id}
                                ...
                                {/* Character count */}
                                {sequence.length > 0 && (
                                    <div className="absolute right-0 top-0 mt-1 mr-2 text-xs text-slate-500 font-medium">
                                        {sequence.length} AA
                                    </div>
                                )}
                            </div>
                            ...
                            <motion.button
                                ...
                                whileHover={{ scale: isValidSequence ? 1.03 : 1 }}
                                whileTap={{ scale: isValidSequence ? 0.97 : 1 }}
                                className={`
                                    relative px-10 py-5 rounded-r-2xl font-semibold text-lg transition-all duration-300
                                    ${isValidSequence
                                        ? 'bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-600 text-white shadow-lg shadow-indigo-500/50 hover:shadow-xl hover:shadow-indigo-500/70'
                            ...
                            >
                                {isValidSequence && (
                                    <motion.div
                                        className="absolute inset-0 rounded-r-2xl bg-gradient-to-r from-white/0 via-white/30 to-white/0"
                                        animate={{ x: ['-100%', '200%'] }}
                                        transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                                    />
                                )}
                                <span className="relative z-10">ANALYZE PEPTIDE</span>
                            </motion.button>
                        </div>
                    </div>
                </motion.div>
